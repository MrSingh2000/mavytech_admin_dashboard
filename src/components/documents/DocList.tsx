...
const DocCard= ({ data }: ItemProps) =>{
    ...
    return (
        <div  data-pr-tooltip="hi" className="filetitle group relative h-48 cursor-pointer overflow-hidden bg-white px-6 pt-5 pb-4 shadow-xl ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl sm:max-w-sm sm:rounded-lg sm:px-6"> 
      
        <button 
            className="absolute top-4 right-4 z-30 text-gray-600 hover:text-gray-900 transition-all duration-300"
            aria-label="Delete card"onClick={() =>
        ...
        <div className="relative z-10 mx-auto max-w-md" onClick={() => handleOpenPdf(createUrl(data.url))}>
            ...
            <Tooltip target=".custom-target-icon" mouseTrack mouseTrackLeft={10} />
                <p className="custom-target-icon truncate w-full"  data-pr-tooltip={data.title}>{data.title}
