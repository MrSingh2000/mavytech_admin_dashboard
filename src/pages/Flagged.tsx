...
import { getApprovalsAction, updateApprovalsAction } from '../redux/slices/approvalSlice';

type Props = {
  setSelectedApproval: React.Dispatch<
    React.SetStateAction<ApprovalType | null>
  >;
};

function Flagged( props: Props) {
  
    
    const items = [
        { label: 'Sales', icon: 'pi pi-home' },
        { label: 'Stores', icon: 'pi pi-chart-line' },
        { label: 'Exibitions', icon: 'pi pi-list' },
        { label: 'News', icon: 'pi pi-inbox' }
    ];
    return (
        ...
        <TabMenu model={items} />
          <div className="py-4">
            ...
            <div className="px-4 py-4 -mx-4 overflow-x-auto sm:-mx-8 sm:px-8">
              <div className="inline-block min-w-full overflow-hidden rounded-lg shadow" style={{
                height: '30rem', 
                overflowY: 'scroll',
                scrollbarWidth: 'none',
...
//           <div className="ml-3">
//             <p className="text-gray-900 whitespace-no-wrap">{formatDate(data.createdAt)}</p>
//           </div>
//         </div>
//       </td>
//       <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
...
//               {data.image? <Image src={createUrl(data.image)} alt="Image" width='84' height='40'   preview className='text-white ' />
//         : 
...
//           </button>: 
//           <button
//           style={{backgroundImage: `url(${pdflogo})`, backgroundSize: '48px 48px'}}
//             type="button"
//             disabled
//             className=" flex justify-center items-center text-white transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 w-12 h-12 rounded-lg "
//           >
